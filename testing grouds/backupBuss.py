...
def manage_buses(action, main_content_frame):
    global unit_type_filter, assigned_route_filter, bus_class_filter, search_filter, status_filter
    ...
    # Function to toggle between "All", "Active", and "Inactive"
    def toggle_status_filter():
        current_value = status_var.get()
        if current_value == "All":
            status_var.set("Active")
        elif current_value == "Active":
            status_var.set("Inactive")
        else:
            status_var.set("All")
        update_table()  # Update the table when the button is toggled
    ...
    # code for Status Filter button
    status_button = ctk.CTkButton(button_frame, textvariable=status_var, fg_color=white, text_color=vio, command=toggle_status_filter, font=("Arial", 16, "bold"), height=35)
    status_button.pack(side="right", pady=5, padx=(5,20))
    ...
    # Treeview table
    columns = ("No.", "Bus ID", "Class", "Fuel Cst.", "MNT Cst.", "Seat No.", "Unit Type", "Assigned Route", "Driver", "Status")
...
def edit_bus(db, tree):
    ...
    if selected_item:  # Check if a row is selected
        ...
        unit_type = ctk.CTkComboBox(edit_bus_frame)
        unit_type.grid(row=0, column=1, padx=10, pady=5, sticky='w')
        # Fetch Unit Types from the database
        try:
            unit_types = db.collection('Unit_type').get()
            ...
            messagebox.showerror("Error", f"Failed to fetch Unit Types: {e}")
            unit_type_names = []
        unit_type.configure(values=unit_type_names)
        unit_type.set("Select Unit Type")  # Set a default prompt

        # --- Number of Seats ---
        ctk.CTkLabel(edit_bus_frame, text="Number of Seats:", text_color="#E2E2B6").grid(row=1, column=0, padx=10, pady=5, sticky='e')
        seats = ctk.CTkEntry(edit_bus_frame, text_color="#191970")
        seats.grid(row=1, column=1, padx=10, pady=5, sticky='w')
        seats.configure(state='readonly')  # Make seats read-only since it's auto-filled
        # --- Bus Class ---
        ctk.CTkLabel(edit_bus_frame, text="Bus Class:", text_color="#E2E2B6").grid(row=2, column=0, padx=10, pady=5, sticky='e')
        bus_class = ctk.CTkComboBox(edit_bus_frame, values=["AC", "NAC"])
        bus_class.grid(row=2, column=1, padx=10, pady=5, sticky='w')
        bus_class.set("Select Bus Class")  # Set a default prompt
        
        # --- Fuel Cost per Km ---
        ctk.CTkLabel(edit_bus_frame, text="Fuel Cost per Km:", text_color="#E2E2B6").grid(row=3, column=0, padx=10, pady=5, sticky='e')
        fuel_cost = ctk.CTkEntry(edit_bus_frame, validate="key", validatecommand=vcmd, text_color="#191970")
        fuel_cost.grid(row=3, column=1, padx=10, pady=5, sticky='w')
        # --- Maintenance Cost ---
        ctk.CTkLabel(edit_bus_frame, text="Maintenance Cost:", text_color="#E2E2B6").grid(row=4, column=0, padx=10, pady=5, sticky='e')
        maintenance_cost = ctk.CTkEntry(edit_bus_frame, validate="key", validatecommand=vcmd, text_color="#191970")
        maintenance_cost.grid(row=4, column=1, padx=10, pady=5, sticky='w')
        # --- Assigned Route ---
        ctk.CTkLabel(edit_bus_frame, text="Assigned Route:", text_color="#E2E2B6").grid(row=5, column=0, padx=10, pady=5, sticky='e')
        assigned_route = ctk.CTkComboBox(edit_bus_frame)
        assigned_route.grid(row=5, column=1, padx=10, pady=5, sticky='w')
        # --- Status ---
        ctk.CTkLabel(edit_bus_frame, text="Status:", text_color="#E2E2B6").grid(row=6, column=0, padx=10, pady=5, sticky='e')
        status_dropdown = ctk.CTkComboBox(edit_bus_frame, values=["Active", "Inactive"])
        status_dropdown.grid(row=6, column=1, padx=10, pady=5, sticky='w')
        ...
        # --- Driver ---
        ctk.CTkLabel(edit_bus_frame, text="Driver:", text_color="#E2E2B6").grid(row=6, column=0, padx=10, pady=5, sticky='e')
        driver = ctk.CTkComboBox(edit_bus_frame)
        driver.grid(row=6, column=1, padx=10, pady=5, sticky='w')
        ...
        driver.configure(values=driver_names)
        driver.set("Select Driver")  # Set initial value to a default prompt

        # --- Function to Update Number of Seats ---
        def update_seat_count(selected_unit):
            if selected_unit and selected_unit != "Select Unit Type":
                try:
                    unit_doc = db.collection('Unit_type').document(selected_unit).get()
                    if unit_doc.exists:
                        no_of_seat = unit_doc.to_dict().get('no_of_seat', '')
                        seats.configure(state='normal')
                        seats.delete(0, ctk.END)
                        ...
                        # Disable unit_type ComboBox after selection
        ...
        # Bind the selection event to update_seat_count function
        unit_type.bind("<<ComboboxSelected>>", lambda event: update_seat_count(unit_type.get()))
        # --- Function to Disable Bus Class after Selection ---
        def disable_bus_class(selected_class):
            if selected_class and selected_class != "Select Bus Class":
                bus_class.configure(state='disabled')
        
        # Bind the selection event to disable_bus_class function
        bus_class.bind("<<ComboboxSelected>>", lambda event: disable_bus_class(bus_class.get()))
        ...
        def load_bus_data():
            try:
                ...
                if doc.exists:
                    data = doc.to_dict()
                    # Populate the fields
                    unit_type.set(data.get('unit_type', 'Select Unit Type'))
                    update_seat_count(data.get('unit_type', ''))
                    bus_class.set(data.get('bus_class', 'Select Bus Class'))
                    # Disable bus_class ComboBox after setting
                    bus_class.configure(state='disabled')
                    fuel_cost.configure(state='normal')
                    fuel_cost.delete(0, ctk.END)
                    fuel_cost.insert(0, data.get('fuel_cost_per_km', ''))
                    fuel_cost.configure(state='normal')  # Assuming users can edit fuel cost
                    maintenance_cost.configure(state='normal')
                    maintenance_cost.delete(0, ctk.END)
                    maintenance_cost.insert(0, data.get('maintenance_cost', ''))
                    maintenance_cost.configure(state='normal')  # Assuming users can edit maintenance cost
                    assigned_route.set(data.get('assigned_route', 'Select Route'))
                    driver.set(data.get('driver', 'Select Driver'))
                    ...
                    # Disable unit_type, seats, and bus_class
                    unit_type.configure(state='disabled')
                    seats.configure(state='disabled')  # Already 'readonly', but ensure
                    bus_class.configure(state='disabled')
            except Exception as e:
                messagebox.showerror("Error", f"Failed to load Bus data: {e}")

        load_bus_data()  # Run function to populate the fields
        ...
        def save_bus():
            ...
            if selected_driver_name == "Select Driver" or not selected_driver_name:
                messagebox.showerror("Error", "Please select a Driver.")
                return
            if status_dropdown.get() not in ["Active", "Inactive"]:
    ...
    # --- Save Button ---
    ctk.CTkButton(edit_bus_frame, text="Save", text_color="#191970",fg_color="#E2E2B6", hover_color="#6EACDA", command=save_bus).grid(row=7, column=0, columnspan=2, pady=20)
...
def refresh_tree(db, tree):
    ...
    #fetch data based on the value fetch from dropdown
    for bus in buses:
        global unit_type_filter, assigned_route_filter, bus_class_filter, search_filter, status_filter
        count =1
        for item in tree.get_children():
        ...
        for bus in buses:
            ...
            if (unit_type_filter == "All" or data.get('unit_type', '') == unit_type_filter) and \
               ...
               (status_filter == "All" or data.get('status', '') == status_filter) and \
               (search_filter in bus_id):  # Check for partial match in Bus ID
                ...
                tree.insert("", ctk.END, values=(
                    ...
                    data['unit_type'], 
                    data.get('assigned_route', ''), 
                    driver,data.get('status', '')), 
                    tags=(row_tag,))
                count += 1